...
function updateFile(file) {
  ...
  selected.textContent = `${file.name} Â· ${formatBytes(file.size)}`;
...
form.addEventListener('submit', async event => {
  ...
  sanitizeButton.innerHTML = 'Scanning artifact <span>â€¦</span>';
  errorMessage.hidden = true;
  const response = await fetch('/api/sanitize', { method: 'POST', body: new FormData(form) });
  const data = await response.json();
  sanitizeButton.innerHTML = 'Sanitize artifact <span>â†’</span>';
  sanitizeButton.disabled = false;
...
function renderResult(data) {
  ...
  status.textContent = report.status === 'PASS' ? 'âœ“ VERIFIED SAFE TO RELEASE' : 'Ã— REVIEW REQUIRED';
